...
module.exports = {
	getMap: async(req, res) => {
	  try {
		const initialLat = 37.591;
		const initialLng = 127.017;
	    ...
	    res.render('map', { mapData: addressData, initialLat, initialLng }); // 클라이언트에서 이 데이터를 활용하여 지도에 핀을 표시합니다.
	  } catch (error) {
	    ...
	    const result = geocodeAddress(add);
            // 주소 DB에서 모든 주소를 조회합니다.
            const addresses = await db.Address.findAll();
	    ...
	    if (result) {
            ...
            // 클라이언트에게 지도와 주소 데이터를 JSON 형태로 전송합니다.
            res.render('map', { mapData: addressData , initialLat, initialLng });
	  } catch (error) {
            console.error('Error fetching addresses:', error);
            res.status(500).send('Error fetching addresses');
          }
